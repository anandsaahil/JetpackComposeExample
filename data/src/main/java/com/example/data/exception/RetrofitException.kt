...
import com.example.data.entity.error.ApiError
...
class RetrofitException(private val _message: String?,
    ...
    private var errorData: ApiError? = null
    ...
    companion object {
        ...
        fun httpErrorWithObject(url: String, response: Response<*>, retrofit: Retrofit): RetrofitException {
            val message = response.code().toString() + " " + response.message()
            val throwable = RetrofitException(message, url, response, Kind.HTTP_422_WITH_DATA, null, retrofit)
            throwable.deserializeServerError(response)
            return throwable
        }
    ...
    fun deserializeServerError(response: Response<*>) {
        if (response.errorBody() != null) {
            try {
                errorData = getErrorBodyAs(ApiError::class.java)
            } catch (e: IOException) {
                Log.d("error","Retrofit deserialization error")
            }
        }
    }
